class Arxiv(object):
    ...
    def arxiv_payload(self, keyword, size = 50, start = 0):
        ...
        response = requests.get('https://arxiv.org/search/', headers=headers, params=params)
    ...
    def arxiv_html_fetch(self, soup):
        ...
        for paper in final_out:
            ...
            paper_link   = paper.find('p', {'class': 'list-title is-inline-block'}).find('a', href =True)['href']
    ...
    def arxiv(self, keyword, max_pages = 5):

        "arxiv function call"

        total_pages = int(max_pages/50)
        all_pages = []
