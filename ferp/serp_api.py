...
class Serp(object):
    ...
    def sengine(self, q):
        ...
        sengine             = GoogleScholarSearch({"q"      : q, 
    ...
    def write_json(self, data, save_path):
        ...
        with open(f'{save_path}/{unique_node_id}.json', 'w', 
                  encoding='utf-8') as f:
    ...
    def filter_url(self, results):
        ...
        for result in fl_result:
            ...
            related_article = re.search(r"q=(.*)\/&scioq", 
                                        result["inline_links"][
                                            "related_pages_link"]
                                       ).group(1)
    ...
    def get_related_pages(self, query):
        ...
        if 'error' in data:
            ...
            for single_url in tqdm(url):
                ...
                result = self.pagination(seng, None, 
                                         folder_path)
    ...
    def get_citations(self, query):
        ...
        url  = [links['inline_links']['cited_by'][
            'serpapi_scholar_link'] 
                for links in data['organic_results']]
        for paper_ in tqdm(url):
            res = self._req_pagination(paper_, 
                            max_pages = None, 
                            save_result = folder_path)
                ...
                if 'error' in result:
                    ...
                    if 'serpapi_pagination' in result:
                        ...
                        if 'next' in pagination:
                            next_page = True
                            next_link = pagination['next_link'] + f'&api_key={self.api_key}'
                        else:
                            next_page = False
            ...
            while has_next_page and page_no <=max_pages:
                ...
                if 'error' in data:
                    ...
                    if 'pagination' in data:
                        ...
                        cengine.params_dict.update(dict(parse_qsl(urlsplit(data.get("serpapi_pagination").get("next")).query)))
                    else:
                        break
